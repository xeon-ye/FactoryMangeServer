...
namespace Coldairarrow.Business.Base_Manage
{
    public class Base_DepartmentBusiness : BaseBusiness<Base_Department>, IBase_DepartmentBusiness, ITransientDependency
    {
        ...
        public List<Base_Department> GetDeptList()
        {
        ...
        public void SaveDepts(IEnumerable<Base_Department> depts)
        {

            depts.ForEach(p =>
            {
                if (p.ParentId == "1") p.ParentId = "";
                p.CreateTime = System.DateTime.Now;
                p.CreatorId = "dd";

                if (GetIQueryable().Count(t => t.Id == p.Id) == 0)
                {   
                    Insert(p);
                }else
                {
